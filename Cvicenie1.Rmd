...
```{r}
xfun::embed_file('docs/Cvicenie1.Html')
```
...
# generate break positions
breaks = round(c(seq(-0.5, 0.5, by=0.05), disp,-disp), digits = 4)
# and labels
labels = as.character(breaks)
...
ggplot(data = acf.y, aes(x = Lag, ymax=ACF, ymin=0))+
  ...
  theme(plot.title = element_text(hjust = 0.5, size=14, face="bold"))+
  scale_y_continuous(limits = c(-0.5, 0.5), breaks = breaks, labels = labels,
                     name = "ACF")
